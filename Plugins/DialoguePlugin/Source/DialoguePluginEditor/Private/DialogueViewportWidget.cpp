...
void SDialogueViewportWidget::SpawnNodes(int32 IdToFocus)
{
	...
	if (!Dialogue->m_AryData.IsValidIndex(0)) // if we're opening the dataAsset for the first time, create the Start node
	{
		FDialogueNode NewNode;
		NewNode.m_nID = 0;
		NewNode.m_Text = FText::FromString(TEXT("Start"));
		bFirstNodeRequiresRepositionning = true;
		Dialogue->m_nNextNodeId = 1;
		Dialogue->m_AryData.Add(NewNode);
	}

	// nodes themselves
	int i = 0;
	for (auto &Node : Dialogue->m_AryData)
	{
		...
		NodeIdsIndexes.Add(Node.m_nID, i);
		i++;
	}
	...
	if (IdToFocus != 0)
	{
		...
		Dialogue->m_nCurrentNodeId = IdToFocus;
		ForceRefresh();
	}
...
void SDialogueViewportWidget::Tick(const FGeometry & AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (bFirstNodeRequiresRepositionning)
	{
		...
		Dialogue->m_AryData[0].m_Coordinates = FVector2D(HCoord, VCoord);
	}
...
FCursorReply SDialogueViewportWidget::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	...
	if (draggedNodeIndex != -1)
	{
		...
		for (auto NodeId : SelectedNodes)
		{
			...
			if (draggedNodeIndex != NodeIndex)
			{
				...
				Dialogue->m_AryData[NodeIndex].m_Coordinates = (MyGeometry.AbsoluteToLocal(CursorEvent.GetScreenSpacePosition()) + draggingOffset) / GetZoomAmount() - panningOffset + (Dialogue->m_AryData[NodeIndex].m_Coordinates - Dialogue->m_AryData[draggedNodeIndex].m_Coordinates);
			}
		}
		...
		Dialogue->m_AryData[draggedNodeIndex].m_Coordinates = (MyGeometry.AbsoluteToLocal(CursorEvent.GetScreenSpacePosition()) + draggingOffset) / GetZoomAmount() - panningOffset;
	...
	if (isSelectingMultiple)
	{
		...
		for (auto Node : Dialogue->m_AryData)
		{
			FVector2D NodeTopLeftPoint = (Node.m_Coordinates + panningOffset) * GetZoomAmount() - (NodeWidgets[nodeIndex]->NodeSize / 2);
			...
			{
				temp_selectedNodeIds.Add(Node.m_nID);
			}
			nodeIndex++;
		}
...
{
	return Dialogue->m_AryData[NodeIdsIndexes.FindRef(Id)];
}
...
void SDialogueViewportWidget::StartDraggingIndex(int32 NodeIndex)
{
	...
	{
		SelectNodes(Dialogue->m_AryData[NodeIndex].m_nID);
	}
	else
	{
		if (!SelectedNodes.Contains(Dialogue->m_AryData[NodeIndex].m_nID))
		{
			DeselectAllNodes();
			SelectNodes(Dialogue->m_AryData[NodeIndex].m_nID);
			ForceRefresh();
...
void SDialogueViewportWidget::OnIsPlayerCommited(ECheckBoxState NewState)
{
	...
	int32 index = NodeIdsIndexes.FindRef(Dialogue->m_nCurrentNodeId);
	Dialogue->m_AryData[index].m_bIsPlayer = (NewState == ECheckBoxState::Checked);
}

ECheckBoxState SDialogueViewportWidget::GetIsPlayer() const
{
	if (Dialogue->m_nCurrentNodeId != -1)
	{
		int32 index = NodeIdsIndexes.FindRef(Dialogue->m_nCurrentNodeId);
		if (Dialogue->m_AryData[index].m_bIsPlayer) return ECheckBoxState::Checked;
	}
...
void SDialogueViewportWidget::OnNodeTextCommited(const FText &InText, ETextCommit::Type)
{
	...
	int32 index = NodeIdsIndexes.FindRef(Dialogue->m_nCurrentNodeId);
	Dialogue->m_AryData[index].m_Text = InText;
}
...
FText SDialogueViewportWidget::GetNodeText() const
{
	if (Dialogue->m_nCurrentNodeId != -1)
	{
		int32 index = Dialogue->m_nCurrentNodeId;
		return Dialogue->m_AryData[index].m_Text;
	}		
...
FMargin SDialogueViewportWidget::GetPos(int32 index)
{
	...
	Output.Left = (Dialogue->m_AryData[index].m_Coordinates.X + panningOffset.X) * GetZoomAmount();
	Output.Top = (Dialogue->m_AryData[index].m_Coordinates.Y + panningOffset.Y) * GetZoomAmount();
...
FReply SDialogueViewportWidget::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (Dialogue->m_bIsLinking)
	{
		...
		Dialogue->m_LinkingCoords = MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
		FVector2D currentMouseCoords = MouseEvent.GetScreenSpacePosition();
		FSlateRect PanelScreenSpaceRect = MyGeometry.GetLayoutBoundingRect();
...
{
	// we're panning
	if (MouseEvent.IsMouseButtonDown(EKeys::RightMouseButton) && !Dialogue->m_bIsLinking && !isSelectingMultiple && draggedNodeIndex == -1)
	{
		isPanning = true;
	}
	// we're selecting multiple
	else if (MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton) && !isPanning && !Dialogue->m_bIsLinking && draggedNodeIndex == -1 && !bBreakingLinksMode)
	{
...
void SDialogueViewportWidget::OnAddNodeClicked()
{
	...
	NewNode.m_Coordinates = clickUpCoords / GetZoomAmount() - panningOffset;
	NewNode.m_Text = FText::FromString(TEXT("Enter some text"));
	Dialogue->m_nNextNodeId++;
	Dialogue->m_AryData.Add(NewNode);
	Dialogue->m_nCurrentNodeId = NewNode.m_nID; //select the new node
	SpawnNodes(NewNode.m_nID);
}
...
int32 SDialogueViewportWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	for (auto Node : Dialogue->m_AryData) //for each dialogue node
	{
		...
		if (isNodeVisible[nodeIndex])
		{
			...
			FVector2D upperLeft = (Node.m_Coordinates + panningOffset) * GetZoomAmount() - (overlaySize / 2.f);
		...
		for (auto Link : Node.m_AryLinks) // for each link
		{
			...
			if (linkIndex != -1)
			{
				...
				// Splines:
				if (Node.m_Coordinates.Y > Dialogue->m_AryData[linkIndex].m_Coordinates.Y)
				{
					FVector2D fromNodeSize = NodeWidgets[nodeIndex]->NodeSize;
					FVector2D startPoint(
						(Node.m_Coordinates.X + panningOffset.X - 8) * GetZoomAmount() + (fromNodeSize.X / 2),
						(Node.m_Coordinates.Y + panningOffset.Y) * GetZoomAmount()
					);

					FVector2D toNodeSize = NodeWidgets[linkIndex]->NodeSize;
					FVector2D endPoint(
						(Dialogue->m_AryData[linkIndex].m_Coordinates.X + panningOffset.X + 3 ) * GetZoomAmount() - (toNodeSize.X / 2),
						(Dialogue->m_AryData[linkIndex].m_Coordinates.Y + panningOffset.Y) * GetZoomAmount()
					);

					// 1. if only one node is selected and if this node is the one selected, then draw blue lines (outgoing)
					if (SelectedNodes.Num() == 1 && SelectedNodes[0] == Node.m_nID)
					{
					...
					else if (Dialogue->m_bDisplayIdleSplines && ZoomLevel > 6)
					{
						FSlateDrawElement::MakeSpline(
							OutDrawElements,
							LayerId + 3,
				...
				else
				{
					...
					LinePoints.Add(FVector2D(
						(Node.m_Coordinates.X + panningOffset.X) * GetZoomAmount(),
						(Node.m_Coordinates.Y + panningOffset.Y - 3) * GetZoomAmount() + fromNodeSize.Y / 2
					));

					// TO
					FVector2D toNodeSize = NodeWidgets[linkIndex]->NodeSize;
					LinePoints.Add(FVector2D(
						(Dialogue->m_AryData[linkIndex].m_Coordinates.X + panningOffset.X) * GetZoomAmount(),
						(Dialogue->m_AryData[linkIndex].m_Coordinates.Y + panningOffset.Y) * GetZoomAmount() - toNodeSize.Y / 2
					));
	...
	if (Dialogue->m_bIsLinking)
	{
		...
		FVector2D fromNodeSize = NodeWidgets[Dialogue->m_nLinkingFromIndex]->NodeSize;
		LinkingPoints.Add(FVector2D(
			(Dialogue->m_AryData[Dialogue->m_nLinkingFromIndex].m_Coordinates.X + panningOffset.X) * GetZoomAmount(), 
			(Dialogue->m_AryData[Dialogue->m_nLinkingFromIndex].m_Coordinates.Y + panningOffset.Y - 3) * GetZoomAmount() + fromNodeSize.Y / 2
		));
		LinkingPoints.Add(Dialogue->m_LinkingCoords);
...
void SDialogueViewportWidget::SelectNodes(TArray<int32> NodesIds) const
{
	...
	{
		Dialogue->m_nCurrentNodeId = -1;
	}
	else if (SelectedNodes.Num() == 1)
	{
		Dialogue->m_nCurrentNodeId = SelectedNodes[0];
		flagForRefresh = true;
	}
...
void SDialogueViewportWidget::SelectUnderCursor() const 
{
	...
	for (auto Node : Dialogue->m_AryData)
	{
		...
		FVector2D NodeTopLeftPoint = (Node.m_Coordinates + panningOffset) * GetZoomAmount() - (NodeWidgets[nodeIndex]->NodeSize / 2);
		FVector2D NodeBottomRightPoint = (Node.m_Coordinates + panningOffset) * GetZoomAmount() + (NodeWidgets[nodeIndex]->NodeSize / 2);
		FSlateRect NodeRect = FSlateRect(NodeTopLeftPoint, NodeBottomRightPoint);

		if (NodeRect.ContainsPoint(clickUpCoords))
		{
			if ( (CurrentSelection == ESelectionType::Type::WithShift || CurrentSelection == ESelectionType::Type::WithCtrl) && !SelectedNodes.Contains(Node.m_nID) )
			{
				SelectedNodes.Add(Node.m_nID);
				NodeWidgets[nodeIndex]->isSelected = true;
			}
			else if (CurrentSelection == ESelectionType::Type::WithCtrl && SelectedNodes.Contains(Node.m_nID))
			{
				SelectedNodes.Remove(Node.m_nID);
				NodeWidgets[nodeIndex]->isSelected = false;
			}
			else if (CurrentSelection == ESelectionType::Type::Default)
			{
				SelectNodes(Node.m_nID);
			}
			ForceRefresh();
			return;
		}
...
{
	if (Dialogue->m_bIsLinking || draggedNodeIndex != -1 || isSelectingMultiple)
...
void SDialogueViewportWidget::CopySelected()
{
	if (SelectedNodes.Num() > 0)
	{
		...
		for (auto & nodeIndex : SelectedNodes)
		{
			...
			Node.m_AryLinks.RemoveAll([this](int32 & LinkedNodeId) { return !SelectedNodes.Contains(LinkedNodeId); });
...
void SDialogueViewportWidget::SerializeEventsConditions(const FDialogueNode& Node, TSharedPtr<FJsonObject> JsonNode)
{
	...
	for (auto & nodeEvent : Node.m_AryEvents)
	{
	...
	for (auto & nodeCondition : Node.m_AryConditions)
	{
		FString BytesAsString = "NOT_IMPLEMENTED_YET";	
...
{
	Node.m_AryConditions.Empty();
	Node.m_AryEvents.Empty();
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObj))
	{
		...
		if (JsonNodes.Num() > 0)
		{
			...
			if (DeserializedNodes.Num() > 0)
			{
				...
				{
					int32 NodeOldId = DeserializedNodes[i].m_nID;
					int32 NodeNewId = Dialogue->m_nNextNodeId;
					Dialogue->m_nNextNodeId++;

					DeserializedNodes[i].m_nID = NodeNewId;

					// as soon as an id has changed, go through all nodes and update links to this node
					for (int j = 0; j < DeserializedNodes.Num(); j++)
					{
						if (DeserializedNodes[j].m_AryLinks.Remove(NodeOldId) > 0)
							DeserializedNodes[j].m_AryLinks.Add(NodeNewId);
					}
				}

				// find the gravitational center of all nodes that we're pasting
				int cycle = 1;
				float sumX = DeserializedNodes[0].m_Coordinates.X;
				float sumY = DeserializedNodes[0].m_Coordinates.Y;
				for (int i = 1; i < DeserializedNodes.Num(); i++)
				{
					float multiplierForPreviousSum = (float)i / ((float)i + 1.f);

					sumX *= multiplierForPreviousSum;
					sumX += DeserializedNodes[i].m_Coordinates.X / ((float)i + 1.f);

					sumY *= multiplierForPreviousSum;
					sumY += DeserializedNodes[i].m_Coordinates.Y / ((float)i + 1.f);
				}
				...
				for (int i = 0; i < DeserializedNodes.Num(); i++)
				{
					DeserializedNodes[i].m_Coordinates.X += AdjustX;
					DeserializedNodes[i].m_Coordinates.Y += AdjustY;
				}

				// add them to Dialogue file
				for (auto & node : DeserializedNodes)
				{
					Dialogue->m_AryData.Add(node);
				}

				// spawn nodes and select them
				SpawnNodes(DeserializedNodes.Num() == 1 ? DeserializedNodes[0].m_nID : -1);
				TArray<int32> temp_nodes_to_select;
				for (auto & node : DeserializedNodes)
				{
					temp_nodes_to_select.Add(node.m_nID);
				}
