 ...
 * of each tab.
 */
class SNPCPaletteEntry : public SCompoundWidget
{
	public:
	SLATE_BEGIN_ARGS(SNPCPaletteEntry)
	{
...
public:
	void Construct(const FArguments& InArgs, const TSharedPtr<const FNPCPaletteItem>& InItem);
	...
	TSharedPtr<const FNPCPaletteItem> Item;
...
class SNPCPaletteModeTool : public SCompoundWidget
{
public:
SLATE_BEGIN_ARGS(SNPCPaletteModeTool)
		{
		}
...
private:
	...
	TSharedRef<SWidget> CreatePlacementGroupTab(const FNPCEntityCategoryInfo& Info);

	/** Generates a widget for the specified item */
	TSharedRef<ITableRow> OnGenerateWidgetForItem(TSharedPtr<FNPCPaletteItem> InItem,
	...
	void UpdateRecentlyPlacedAssets(const TArray<FNPCPlacementInfo>& RecentlyPlaced);
...
private:
	...
	typedef TTextFilter<const FNPCPaletteItem&> FPlacementAssetEntryTextFilter;
	...
	TArray<TSharedPtr<FNPCPaletteItem>> FilteredItems;

	/** The name of the currently active tab (where no search is active) */
	FName ActiveTabName;

	/** List view that shows placeable items */
	TSharedPtr<SListView<TSharedPtr<FNPCPaletteItem>>> ListView;
public://New 2021
	//
	TSharedPtr<SObjectPropertyEntryBox> m_AssetEntry;

	void OnAssetDropped( const FAssetData& asset);
