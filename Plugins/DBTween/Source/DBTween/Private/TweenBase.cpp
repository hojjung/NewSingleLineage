...
void UTweenBase::Stop()
{
	SetReadyToDestroy();
}

void UTweenBase::SetOvershootOrAmplitudeAndPeriod(float OvershootOrAmplitudeParam, float PeriodParam)
{
	easePeriod = PeriodParam;
	easeOvershootOrAmplitude = OvershootOrAmplitudeParam;
}

void UTweenBase::Reset()
{
	CurTime = 0;
	bReverse = false;
	bool bIsPaused = Paused;
	Paused = false;
	isInit = true;
	if(Finished)
	{
...
void UTweenBase::Init(float durTime)
{
	FTickHandle updateCall = FTickHandle();
	updateCall.BindUObject(this, &UTweenBase::UpdateFunc);
	TotalTime = durTime;
	UpdateKey = ADBTweenUpdateManager::RegisterUpdateCall(updateCall);
}

void UTweenBase::SetReadyToDestroy()
{
	if (Finished)return;
	Finished = true;
	ADBTweenUpdateManager::RemoveUpdateCall(UpdateKey);
...
void UTweenBase::UpdateFunc(float deltaTime)
{
...
void UTweenBase::SetLooping(bool bIsLoop)
{
	bLoop = bIsLoop;
}

float UTweenBase::Evaluate()
{
...
float UTweenBase::BounceEaseIn(float time, float duration, float unusedOvershootOrAmplitude, float unusedPeriod)
{
	return 1.0 - BounceEaseOut(duration - time, duration, -1.0, -1.0);
}

float UTweenBase::BounceEaseInOut(float time, float duration, float unusedOvershootOrAmplitude, float unusedPeriod)
{
...
float UTweenBase::BounceEaseOut(float time, float duration, float unusedOvershootOrAmplitude, float unusedPeriod)
{
	time = time / duration;
	if (time < 0.3636364f)
	{
